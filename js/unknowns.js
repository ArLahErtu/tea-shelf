...
export function renderUnknowns() {
  ...
  unknowns.forEach((r) => {
    ...
    note.append(document.createTextNode(daysLeft <= 1
      ? `⚠️ Неизвестный чай #${r.unknown_number} будет удалён завтра. Журнал будет экспортирован. `
      : `⚠️ Неизвестный чай #${r.unknown_number} будет удалён через ${daysLeft} дн. Хотите опознать его? `));
...
function unknownCard(r) {
  ...
  node.innerHTML = `
    ...
    ${r.photo_url ? `<img class="unknown-photo" src="${escapeHtml(r.photo_url)}" alt="" loading="lazy">` : ''}
...
// ---------- Фото (компактный загрузчик в тот же бакет) ----------
const MIME_BY_EXT = {
...
async function uploadUnknownPhoto(file) {
  ...
  const doUpload = async (f, m, e) => {
    const path = `unknown/unknown-${Date.now()}.${e}`;
    const { error } = await supabase.storage.from('tea-photos')
      .upload(path, f, { cacheControl: '3600', upsert: false, contentType: m });
    if (error) return { error };
    const { data } = supabase.storage.from('tea-photos').getPublicUrl(path);
  ...
  try {
    ...
    const r2 = await doUpload(jpeg, 'image/jpeg', 'jpg');
    if (!r2.error) {
      showToast('ℹ️ Хранилище не приняло формат — сохранено как JPG');
      return r2.publicUrl;
    }
  } catch (e) { /* падаем в ошибку ниже */ }
  ...
  section.innerHTML = `
    <div class="panel-head-row">
      ...
      <span class="head-actions">
        <button class="btn btn-outline btn-sm" id="unknownArchiveBtn" type="button">📚 Архив</button>
  ...
  <div class="overlay" id="unknownOverlay" hidden>
    <div class="modal" role="dialog" aria-modal="true" aria-labelledby="unknownFormTitle">
      ...
      <form id="unknownForm" novalidate>
        ...
        <p class="warn-note">⚠️ Неизвестные чаи удаляются после завершения, если не будут опознаны. Буферный период: 30 дней.</p>
...
export async function initUnknowns() {
  if (!injected) {
    ...
    // ЭТАП 4: архив журналов + CSV
    $('#unknownArchiveBtn').addEventListener('click', openArchiveOverlay);
    $('#archiveOvClose').addEventListener('click', () => closeOverlay($('#unknownArchiveOverlay')));

    // Клики по карточкам
    $('#unknownsGrid').addEventListener('click', (e) => {
      const card = e.target.closest('.unknown-card');
      if (!card) return;
      const row = unknowns.find((r) => r.id === card.dataset.unknownId);
      if (!row) return;
